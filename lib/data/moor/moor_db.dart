...
@DriftDatabase(tables: [MoorRecipe, MoorIngredient], daos: [RecipeDao, IngredientDao])
...
class RecipeDao extends DatabaseAccessor<RecipeDatabase> with _$RecipeDaoMixin {
  ...
  Future deleteRecipe(int id) => Future.value((delete(moorRecipe)..where((tbl) => tbl.id.equals(id))).go());
}
...
LazyDatabase _openConnection() {
  return LazyDatabase(() async {
    ...
    final file = File( p.join(dbFolder.path, 'recipes_db.sqlite') );
